...
(function () {
    ...
    const STEAM_SCALE_TEMPLATE = "<strong class=\"f_12px f_Bold l_Right steam_temp steam_scale\"></strong>";
            ...
            if (helper_config.buffSellThreshold >= buff_sell_num) {
                ...
                if (orderList.success) {
                    ...
                    if (orderList.buy_order_table == "") {
                        ...
                        steamOrderScale = getScale(buff_sell_min_price, steam_highest_buy_order.cny);
                        if (helper_config.displaySoldNumber) {
                            ...
                            orderNumber = $(orderList.buy_order_summary)[0].innerText;
                ...
                $(target).after($(STEAM_ORDER_SCALE_TEMPLATE).text("┊"));
                paintingGradient(scale, target, 4, STEAM_SCALE_TEMPLATE);
                let score = "99";
                if (scale && steamOrderScale && orderNumber) {
                    score = (parseFloat(scale) + 10 * ((parseFloat(steamOrderScale) - parseFloat(scale)) ** 3) + Math.exp(- parseFloat(orderNumber) / 62)).toFixed(2);
                }
                $(good).attr("data-buff-sort", score);
                paintingGradient(score, target, 3);
